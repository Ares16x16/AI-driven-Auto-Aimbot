...
def compose_E(RT1, RT2):
    ...
    # compute the relative rotation R
    R1 = RT1[:, :3]
    T1 = RT1[:, 3]
    R2 = RT2[:, :3]
    T2 = RT2[:, 3]
    R = np.dot(R2, R1.T)
    # compute the relative translation T
    T = T2 - np.dot(R, T1)
    T_skew = np.array([[0, -T[2], T[1]], [T[2], 0, -T[0]], [-T[1], T[0], 0]])
    # compose E from R and T
    E = np.dot(T_skew, R)
    return E
...
def check_E(img1, file1, img2, file2, E, K1, K2):
    ...
    print(
        "please select points on {} using left-click, end with right-click...".format(
            file1
        )
    )
...
def main():
    parser = argparse.ArgumentParser(description="Epipolar")
    parser.add_argument(
        "-i1",
        "--image1",
        type=str,
        default="left_good.jpg",
